...
def convert_pptx_to_slide_images(pptx_path: Path) -> List[Dict]:
    ...
    try:
        ...
        try:
            # Method 1: Try to read the presentation and create visual representations
            ...
            for i, slide in enumerate(prs.slides):
                ...
                display_content = slide_content[:8]  # Show first 8 content lines
                
                # Create slide visualization
            ...
            logger.warning(f"Primary slide conversion failed: {e}")
            # Fallback: Create generic slide representations
            for i in range(6):  # Assume reasonable number of slides
                fig, ax = plt.subplots(figsize=(16, 9))
                ax.axis('off')
                ax.set_facecolor('#2d3748')
                
                # Create a generic slide representation
                ax.text(0.5, 0.6, f"Slide {i+1}", 
                       ha='center', va='center', fontsize=24, color='white', weight='bold',
                       transform=ax.transAxes)
                ax.text(0.5, 0.4, "Content will be displayed in actual PowerPoint", 
                       ha='center', va='center', fontsize=14, color='#a0aec0',
                       transform=ax.transAxes)
                ...
                slide_images.append({
                    ...
                    "image_data": f"data:image/png;base64,{img_base64}",
                    "title": f"Slide {i + 1}",
                    "content": [f"Slide {i+1} - Generated Content"]
                })
...
def run_generation_pipeline():
    ...
    try:
        ...
        # Run generation.py as a subprocess with proper encoding and timeout
...
def generate_powerpoint(request: PPTGenerationRequest) -> dict:
    ...
    try:
        ...
        # NEW: Generate slide images for preview
